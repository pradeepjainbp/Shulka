...
export const metadata = {
  title: 'Incoming Invoices — Shulka',
}

// ---------------------------------------------------------------------------
// Money formatter — tabular-nums, Indian lakh comma (Sacred Rule §17)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Status pill — same pattern as sales/page.tsx
// ---------------------------------------------------------------------------
...
export default async function IncomingInvoicesPage({
    ...
    return (
      <main className="min-h-screen bg-surface p-6">
        <div className="max-w-2xl mx-auto rounded-lg border border-border bg-raised p-10 flex flex-col items-center gap-4 text-center">
          <Inbox className="h-10 w-10 text-ink-muted" strokeWidth={1.5} />
          <div className="space-y-1">
            <p className="font-medium text-ink">No business found</p>
            <p className="text-sm text-ink-muted">
              Create a business before you can view incoming invoices.
            </p>
          </div>
          <Button asChild>
  ...
  // Fetch incoming quarantined invoices:
  //   - linked_to_business_id = myBusinessId (sent to us)
  //   - status != 'cancelled'
  //   - no active trusted business_trusts row for (truster=me, trusted=sender)
  const rows = await db
    .select({
      id: salesInvoices.id,
      ...
      senderBusinessId: salesInvoices.businessId,
    ...
    .where(
      and(
        eq(salesInvoices.linkedToBusinessId, biz.id),
        ...
        notExists(
          db
            ...
            .where(
              and(
                eq(businessTrusts.trusterBusinessId, biz.id),
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-ink-muted mt-0.5">
            Invoices sent to you via the Shulka network
          </p>
        </div>
        ...
        {rows.length === 0 ? (
          ...
          <div className="rounded-lg border border-border bg-raised p-12 flex flex-col items-center gap-5 text-center">
            <div className="h-16 w-16 rounded-full bg-surface flex items-center justify-center">
              <Inbox className="h-7 w-7 text-ink-muted" strokeWidth={1.5} />
            </div>
            <div className="space-y-1.5 max-w-xs">
              <p className="font-semibold text-ink text-[16px]">No incoming invoices yet</p>
              <p className="text-sm text-ink-muted">
                Share Shulka with your suppliers — invoices auto-link when they join.
              </p>
            </div>
          </div>
        ) : (
          ...
          <div className="rounded-lg border border-border bg-raised overflow-hidden">
            {/* Table header */}
            <div className="hidden sm:grid grid-cols-[minmax(0,2fr)_minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,auto)] gap-4 px-5 py-3 border-b border-border bg-surface">
            ...
            <div className="divide-y divide-border">
              {rows.map((inv) => (
                <div
                  key={inv.id}
                  className="flex flex-col sm:grid sm:grid-cols-[minmax(0,2fr)_minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,auto)] gap-2 sm:gap-4 sm:items-center px-5 py-4"
                >
                  {/* Invoice number */}
                  <div>
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-ink truncate">{inv.senderBusinessName}</p>
                    {inv.partyName && (
                      <p className="text-xs text-ink-muted truncate">{inv.partyName}</p>
                    )}
                  </div>
                  ...
                  <div>
                    <TrustButton
                      invoiceId={inv.id}
                      businessId={biz.id}
                      senderName={inv.senderBusinessName}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="px-5 py-3 border-t border-border bg-surface text-xs text-ink-muted">
              {rows.length} {rows.length === 1 ? 'invoice' : 'invoices'} pending
            </div>
          </div>
        )}
      </div>
